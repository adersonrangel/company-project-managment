...
public class ActualizarProyectoValidator : AbstractValidator<ActualizarProyectoRequest>
{
    ...
    {
        RuleFor(x => x)
            .Must(x => x.Nombre is not null || x.FechaHabilitacion is not null || x.EstadoHabilitacion is not null)
            .WithMessage("Se requiere al menos un campo para actualizar (Nombre, FechaHabilitación o EstadoHabilitación).")
            .WithName("Request");
        ...
        When(x => x.FechaHabilitacion is not null, () =>
        {
